...
-- old auction
insert into auction(auctioncode, auctiontype, name, item_id, startdate, enddate, baseprice, reserveprice, deltaprice, timeextension) select 'DUMMY-AUCTION-3', 'ENGLISH', 'Vintage Car (good condition)', item_id, subdate(current_timestamp, interval 1 month), subdate(current_timestamp, interval 1 day), 20.00, 870.00, 5.50, 3 from item where code = 'DUMMY-ITEM-1';

-- future auction
insert into auction(auctioncode, auctiontype, name, item_id, startdate, enddate, baseprice, reserveprice, deltaprice, timeextension) select 'DUMMY-AUCTION-4', 'ENGLISH', 'Dreadnought Acoustic Guitar (less than 2 years old)', item_id, adddate(current_timestamp, interval 1 day), adddate(current_timestamp, interval 1 month), 10000.00, 100000.00, 100.00, 5 from item where code = 'DUMMY-ITEM-2';
insert into auction(auctioncode, auctiontype, name, item_id, startdate, enddate, baseprice, reserveprice, deltaprice, timeextension) select 'DUMMY-AUCTION-5', 'ENGLISH', 'German Vintage Luxary Car', item_id, adddate(current_timestamp, interval 2 day), adddate(current_timestamp, interval 6 day), 50000.00, 1000000.00, 1000.00, 10 from item where code = 'DUMMY-ITEM-1';
insert into auction(auctioncode, auctiontype, name, item_id, startdate, enddate, baseprice, reserveprice, deltaprice, timeextension) select 'DUMMY-AUCTION-6', 'ENGLISH', 'Coffee Vending Machine', item_id, adddate(current_timestamp, interval 1 month), adddate(current_timestamp, interval 2 month), 10000.00, 100000.00, 100.00, 5 from item where code = 'DUMMY-ITEM-3';
